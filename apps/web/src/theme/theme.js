...
const clampHex = (v) => Math.max(0, Math.min(255, v))
const hex = (v) => clampHex(v).toString(16).padStart(2, '0')
const parseHex = (h) => {
...
const lighten = (color, amount = 0.15) => {
...
const getCustomPrimary = () => {
  try {
    const raw = window.localStorage.getItem('celler.customization')
    if (!raw) return null
    const obj = JSON.parse(raw)
    const p = obj?.primaryColor
    return parseHex(p) ? p : null
  } catch {
    return null
  }
}
...
export const createSaymonCellTheme = (isDarkMode) => {
  const customPrimary = getCustomPrimary() || colors.red.main
  const accentAlpha = (a) => alpha(customPrimary, a)
  return createTheme({
    breakpoints: {
      values: {
        xs: 0,
    ...
    palette: {
      ...
      primary: {
        main: customPrimary,
        light: lighten(customPrimary, 0.2),
        dark: darken(customPrimary, 0.2),
        contrastText: '#ffffff',
      },
      secondary: {
        ...
        styleOverrides: {
          root: ({ theme }) => ({
            ...
            '&::before': {
              ...
              width: '100%',
              height: '100%',
              background: ({ theme }) => `linear-gradient(90deg, transparent, ${alpha(theme.palette.primary.main, 0.15)}, transparent)`,
      ...
      MuiListItemButton: {
        styleOverrides: {
          root: ({ theme }) => ({
            ...
            '&.Mui-selected': {
              background: ({ theme }) => `linear-gradient(135deg, ${alpha(theme.palette.primary.main, 0.09)}, ${alpha(theme.palette.primary.light, 0.05)})`,
          ...
          root: ({ theme }) => ({
            '& .MuiOutlinedInput-root': {
              ...
              '&.Mui-focused fieldset': {
                borderColor: customPrimary,
                boxShadow: `0 0 0 3px ${accentAlpha(0.125)}`,
              },
              '& input': {
                fontSize: 'clamp(0.875rem, 1.5vw, 1rem)',
              },
